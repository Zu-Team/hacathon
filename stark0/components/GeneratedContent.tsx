  ...
  useEffect(() => {
    ...
    const handleClick = (event: MouseEvent) => {
      ...
      if (targetElement && targetElement.dataset.interactionId) {
        ...
        } else if (targetElement.dataset.valueFromMultiple) {
          const ids = targetElement.dataset.valueFromMultiple
            .split(',')
            .map((id) => id.trim());
          const values: {[key: string]: string} = {};
          ids.forEach((id) => {
            const inputElement = document.getElementById(id) as
              | HTMLInputElement
              | HTMLTextAreaElement;
            if (inputElement) {
              values[id] = inputElement.value;
            }
          });
    ...
    if (!isLoading) {
      if (htmlContent !== processedHtmlContentRef.current) {
        ...
        // Fix: Explicitly type `oldScript` as `HTMLScriptElement` to resolve 'unknown' type errors.
